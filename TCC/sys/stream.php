	...
	while($onlines = $pegaOnline->fetch()){
		...
		if(!in_array($userOnline, $blocks)){
			if($agora >= $onlines['limite']){
				$usersOn[] = array('id' => $onlines['id'], 'nome' => utf8_decode($onlines['nome']), 'foto' => $foto, 'status' => 'off');
			}else{
				$usersOn[] = array('id' => $onlines['id'], 'nome' => utf8_decode($onlines['nome']), 'foto' => $foto, 'status' => 'on');
			}
		}
	}
